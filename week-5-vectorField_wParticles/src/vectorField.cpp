...
void vectorField::draw(){
    ...
    for (int i = 0; i < fieldWidth; i++){
        for (int j = 0; j < fieldHeight; j++){
            ...
            float px = 	i * scalex;
            float py = 	j * scaley;
            float px2 = px + field[pos].x;
            float py2 = py + field[pos].y;
			...
			ofLine(px - line.x*length*0.0001, py - line.y*length*0.0001, px + line.x*length*0.0001, py + line.y*length*0.0001);
