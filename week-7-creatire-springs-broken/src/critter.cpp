...
void critter::setup(ofColor color, float size) {
	...
	// all critters hop at different rates
...
void critter::update() {
	...
	for (int i = 10; i < 14; i++) {
		...
		springs[i].distance = sin(ofGetElapsedTimef()*8 * sinOffset);
	}
...
void critter::draw() {
	...
	// different method for drawing the critter with different
...
void critter::createSpring(particle * p1, particle * p2) {
...
void critter::repelMouse(float x, float y) {
	mouseX = x;
	mouseY = y;
}