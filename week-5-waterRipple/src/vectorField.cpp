...
void vectorField::randomizeField(float scale){
	for (int i = 0; i < nTotal; i++){
        ...
        float x = (float)(ofRandom(-100,100)) * scale;
        float y = (float)(ofRandom(-100,100)) * scale;
...
void vectorField::noiseField( float _scale  ,float _speed, float _turbulence, bool _signed){
    ...
    for (int i = 0; i < cols; i++){
        for (int j = 0; j < rows; j++){
            ...
            float u, v;
            if (_signed){
                u = ofSignedNoise(t + PI, normx * _turbulence + TWO_PI, normy * _turbulence + TWO_PI);
                v = ofSignedNoise(t - PI, normx * _turbulence - TWO_PI, normy * _turbulence + TWO_PI);
            } else {
                u = ofNoise(t + PI, normx * _turbulence + TWO_PI, normy * _turbulence + TWO_PI);
                v = ofNoise(t - PI, normx * _turbulence - TWO_PI, normy * _turbulence + TWO_PI);
            }
...
void vectorField::draw(){
    ...
    for (int i = 0; i < cols; i++){
        for (int j = 0; j < rows; j++){
            ...
            float px = 	i * scalex;
            float py = 	j * scaley;
            float px2 = px + buffer[activeBuffer%2][pos].x * 15;
            float py2 = py + buffer[activeBuffer%2][pos].y * 15;
			
            float angle = atan2(buffer[activeBuffer%2][pos].y,buffer[activeBuffer%2][pos].x);
            
       //     ofColor color;
//            color.set(255,0,255,20);
//            color.setHue(ofMap(angle,-TWO_PI, PI,0,255));
//
//			
			...
			ofCircle(px, py, sqrt(px2-px + py2-py)*5);//
//			ofVec2f line;
//			line.set(px2-px, py2-py);
//			float length = line.length();
//			line.normalize();
//			line.rotate(360);  // these are angles in degrees
//			ofLine(px - line.x*length*0.0002, py - line.y*length*0.0002, px + line.x*length*0.0002, py + line.y*length*0.0002);
}
    }
    ofPopStyle();
}
...
void vectorField::addInwardCircle(float x, float y, float radius, float strength){
    ...
    for (int i = startx; i < endx; i++){
        for (int j = starty; j < endy; j++){
			...
			if (distance < 0.000001) distance = 0.000001;  // since we divide by distance, do some checking here, devide by 0 is BADDDD
    ...
    for (int i = startx; i < endx; i++){
        for (int j = starty; j < endy; j++){
			...
			if (distance < fieldRadius){
				...
				float pct = 100.0f - (distance / fieldRadius);
